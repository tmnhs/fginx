...
func NewHuaWeiObsClient() (client *obs.ObsClient, err error) {
	return obs.New(global.GV_CONFIG.HuaWeiObs.AccessKey, global.GV_CONFIG.HuaWeiObs.SecretKey, global.GV_CONFIG.HuaWeiObs.Endpoint)
}
...
func (o *_obs) UploadFile(file *multipart.FileHeader) (filename string, filepath string, err error) {
	...
	input := &obs.PutObjectInput{
		PutObjectBasicInput: obs.PutObjectBasicInput{
			ObjectOperationInput: obs.ObjectOperationInput{
				Bucket: global.GV_CONFIG.HuaWeiObs.Bucket,
				Key:    filename,
			},
	...
	filepath = global.GV_CONFIG.HuaWeiObs.Path + "/" + filename
