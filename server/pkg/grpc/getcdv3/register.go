...
func RegisterEtcd(schema, etcdAddr, myHost string, myPort int, serviceName string, ttl int) error {
	...
	fmt.Println("", "RegisterEtcd, ", schema, etcdAddr, myHost, myPort, serviceName, ttl)
	if err != nil {
	...
	go func() {
		for {
			...
			case pv, ok := <-kresp:
				if ok == true {
					fmt.Println("", "KeepAlive kresp ok", pv)
				} else {
					fmt.Println("", "KeepAlive kresp failed", pv)
					...
					for {
						...
						resp, err := cli.Grant(ctx, int64(ttl))
						if err != nil {
							fmt.Errorf("", "Grant failed ", err.Error())
							continue
						}

						if _, err := cli.Put(ctx, serviceKey, serviceValue, clientv3.WithLease(resp.ID)); err != nil {
							fmt.Println("", "etcd Put failed ", err.Error(), serviceKey, serviceValue, resp.ID)
							continue
						} else {
							fmt.Println("", "etcd Put ok", serviceKey, serviceValue, resp.ID)
						}
